...
public class PcConnection
{
    ...
    private void processRequest(String method, String reqId, InputStream is) throws IOException
    {
        ...
        else if (HddConstants.METHOD_RENAME.equals(method))
        {
            processRename(reqId, is);
        }
    ...
    private void processRename(String reqId, InputStream is) throws IOException
    {
        String[] params = new String[0];
        params = HddUtil.deserialize(is, params.getClass());
        Boolean resp = hddAccess.rename(params[0], params[1], params[2]);
        doReply(reqId, resp);
    }
    ...
    private void processWriteStream(String reqId, InputStream is) throws IOException
    {
        ...
        LOGGER.debug("IS bytes {}",is.available());
	...
	public void stampProgress() {
		oldCommands = numCommands;
		hddAccess.stampProgress();
	}

	public boolean progressed() {
		return oldCommands != numCommands || hddAccess.progressed();
	}
