...
/**
 * Implements the harvest profile for olympus-TIRF. It harvest a tif file and nothing else.
 * 
 * @version $Rev: 29 $
 */
public class TirfHarvester implements InstrumentHarvester
{
    @Autowired
    ...
    private boolean aTifHasAlreadyBeenProcessed;

    @Override
    public FileHarvester getHarvesterFor(String path)
    {
        if (!aTifHasAlreadyBeenProcessed && path.toLowerCase().endsWith(".tif"))
        {
            aTifHasAlreadyBeenProcessed = true;
            // note how we override the path and use the toDir as the target URL
            return new TirfTifHarvester(executorService, urlCreator.getUrl(toDir), this);
        }
        else
        {
            return null;
