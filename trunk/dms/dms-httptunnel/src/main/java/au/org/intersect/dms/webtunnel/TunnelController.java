...
public class TunnelController
{
    ...
    private TunnelMap<TunnelJobTracker> jobs = new TunnelMap<TunnelJobTracker>();
    ...
    public void hdd(HttpServletRequest request, HttpServletResponse response) throws IOException,
        EncryptionAgentException
    {
        if (LOGGER.isDebugEnabled())
        {
            debugHeaders("HDD", request);
        }
        // TODO CHECKSTYLE-OFF: IllegalInstantiation
        String jobId = decryptJobId(request);

        String method = request.getHeader(HddConstants.METHOD_HEADER);
        TunnelJobTracker tracker = getOrCreateJobTracker(jobId);
        try
        {
            tracker.processRequest(method, request.getInputStream(), response);
        }
        finally
        {
            if (HddConstants.METHOD_FINALIZE.equals(method))
            {
                removeExchangeQueue(jobId);
            }
        }
    }
    ...
    private void removeExchangeQueue(String jobId)
    {
        LOGGER.info("remove #" + jobId);
        jobs.remove(jobId);
    }
