...
public class MicroCTLogHarvesterTest
{
    private static final Logger LOGGER = LoggerFactory.getLogger(MicroCTLogHarvesterTest.class);
    ...
    public void testHarvest() throws IOException
    {
        ...
        MetadataAccumulator accumulator = new MetadataAccumulator()
        {
            ...
            public void addMetadataItem(MetadataEventItem mdItem)
            {
                ...
                assertEquals(MetadataSchema.MICRO_CT, mdItem.getSchema());
                // test that it contains an expected value there
                assertTrue(mdItem.getMetadata().contains("Skyscan1172"));
                assertTrue(mdItem.getMetadata().contains("Reconstruction duration per slice (seconds)"));
                ...
                assertTrue("Dates conversion from EST to UTC is incorrect",
                        mdItem.getMetadata().contains("2010-05-12T07:04:19Z"));

                // test that it does not have one of Allan's exclusions
                assertFalse(mdItem.getMetadata().contains("C:\\Downloads\\Skyscan\\nreconLocal\\Local"));
                assertFalse(mdItem.getMetadata().contains("Draw Scales"));
            }
        };
        MicroCTLogHarvester harvester = new MicroCTLogHarvester(Executors.newSingleThreadExecutor(), url, accumulator);
        InputStream is = getClass().getResourceAsStream("/micro-ct/C4-01_.log");
